...
if [[ "${1:-}" == "--dry-run" ]]; then
  DRY_RUN=true
  echo "🧪 Dry-run mode — no files will be uploaded or committed"
fi
...
if $DRY_RUN; then
  ...
  echo "  Would upload 9 zip files to GitHub Releases"
else
...
if $DRY_RUN; then
  echo "  Would upload: archive-v*.duckdb + 9 photo zips"
else
