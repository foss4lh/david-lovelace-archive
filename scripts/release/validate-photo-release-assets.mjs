...
async function main() {
  ...
  try {
    ...
    const mappingByPath = new Map(photoUrls.map((entry) => [normalizePathForUrl(entry.path), entry]));
    ...
    for (const entry of photoUrls) {
      ...
      if (thumbMember && !zipFiles.has(thumbMember)) {
        missingZipMembers.push(`mapped thumb missing in zip '${entry.thumb_url}' for '${normalizedPath}'`);
      }
    }

    const uniqueMissingZipMembers = [...new Set(missingZipMembers)];
    if (missingMappings.length || wrongUrls.length || staleMappings.length || uniqueMissingZipMembers.length) {
      fail('photo URL mapping and release zip are out of sync', [
        ...
        ...staleMappings.slice(0, 10).map((item) => `mapping path not present in manifest '${item}'`),
